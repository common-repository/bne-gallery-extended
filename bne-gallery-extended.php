			...
			*/
			if( $atts['display'] == 'carousel' || $atts['display'] == 'carrousel' ) {
				...
				$output .= '<div id="bne-carousel-'.$rand_id.'" class="bne-gallery-extended bne-gallery-carousel-wrapper clearfix">';
					...
					wp_add_inline_script( 'roundabout', 
							...
							foreach( $image_ids as $id ) {
								$output .= '<li class="slide gallery-item image-id-'.$id.'">';
									...
									if( !empty( $atts['link'] ) && 'file' === $atts['link'] ) {
										$output .= wp_get_attachment_link( $id, $atts['size'], false, false, false, array( 'loading' => 'eager' ) );
									// Link: None
									} elseif( !empty( $atts['link'] ) && 'none' === $atts['link'] ) {
										$output .= wp_get_attachment_image( $id, $atts['size'], false, array( 'loading' => 'eager' ) );
									// Link: Attachment
									} else {
										$output .= wp_get_attachment_link( $id, $atts['size'], true, false, false, array( 'loading' => 'eager' ) );
									}
						...
						$output .= '<div class="gallery-single gallery-item image-id-'.$id.' col-'.$atts['columns'].'-masonry" style="margin-bottom: '.$atts['gutter'].'px; width: '.$grid_col_width.';">';
							...
							if( $atts['caption'] == 'true' ) {
								...
								if( $meta['caption'] ) {
									...
									// Link: File
									if( !empty( $atts['link'] ) && 'file' === $atts['link'] ) {
										$output .= wp_get_attachment_link( $id, false, false, false, $caption, array( 'loading' => 'eager' ) );
									// Link: None
									} elseif( !empty( $atts['link'] ) && 'none' === $atts['link'] ) {
										$output .= $caption;
									// Link: Attachment
									} else {
										$output .= wp_get_attachment_link( $id, false, true, false, $caption, array( 'loading' => 'eager' ) );
									}
